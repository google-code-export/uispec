...
@class UISPEC_AsyncReadPacket;
@class UISPEC_AsyncWritePacket;

extern NSString *const UISPEC_AsyncSocketException;
extern NSString *const UISPEC_AsyncSocketErrorDomain;
...
**/
- (void)onSocket:(UISPEC_AsyncSocket *)sock willDisconnectWithError:(NSError *)err;

/**
 * Called when a socket disconnects with or without error.  If you want to release a socket after it disconnects,
 * do so here. It is not safe to do that during "onSocket:willDisconnectWithError:".
**/
- (void)onSocketDidDisconnect:(UISPEC_AsyncSocket *)sock;

/**
 * Called when a socket accepts a connection.  Another socket is spawned to handle it. The new socket will have
 * the same delegate and will call "onSocket:didConnectToHost:port:".
**/
- (void)onSocket:(UISPEC_AsyncSocket *)sock didAcceptNewSocket:(UISPEC_AsyncSocket *)newSocket;

/**
 * Called when a new socket is spawned to handle a connection.  This method should return the run-loop of the
 * thread on which the new socket and its delegate should operate. If omitted, [NSRunLoop currentRunLoop] is used.
**/
- (NSRunLoop *)onSocket:(UISPEC_AsyncSocket *)sock wantsRunLoopForNewSocket:(UISPEC_AsyncSocket *)newSocket;
...
**/
- (BOOL)onSocketWillConnect:(UISPEC_AsyncSocket *)sock;

/**
 * Called when a socket connects and is ready for reading and writing.
 * The host parameter will be an IP address, not a DNS name.
**/
- (void)onSocket:(UISPEC_AsyncSocket *)sock didConnectToHost:(NSString *)host port:(UInt16)port;

/**
 * Called when a socket has completed reading the requested data into memory.
 * Not called if there is an error.
**/
- (void)onSocket:(UISPEC_AsyncSocket *)sock didReadData:(NSData *)data withTag:(long)tag;
...
**/
- (void)onSocket:(UISPEC_AsyncSocket *)sock didReadPartialDataOfLength:(CFIndex)partialLength tag:(long)tag;

/**
 * Called when a socket has completed writing the requested data. Not called if there is an error.
**/
- (void)onSocket:(UISPEC_AsyncSocket *)sock didWriteDataWithTag:(long)tag;

@end

@interface UISPEC_AsyncSocket : NSObject
{
	...
	UISPEC_AsyncReadPacket *theCurrentRead;
	NSTimer *theReadTimer;
	NSMutableData *partialReadBuffer;
	
	NSMutableArray *theWriteQueue;
	UISPEC_AsyncWritePacket *theCurrentWrite;
