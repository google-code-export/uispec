...
#import "UISPEC_HTTPAuthenticationRequest.h"
#import "DDNumber.h"
#import "DDRange.h"
#import "DDData.h"
...
@interface UISPEC_HTTPConnection (PrivateAPI)
...
@implementation UISPEC_HTTPConnection
...
- (id)initWithAsyncSocket:(UISPEC_AsyncSocket *)newSocket forServer:(UISPEC_HTTPServer *)myServer
{
	if(self = [super init])
	{
		...
		[asyncSocket readDataToData:[UISPEC_AsyncSocket CRLFData]
...
- (NSString *)generateNonce
{
	...
	[NSTimer scheduledTimerWithTimeInterval:NONCE_TIMEOUT
									 target:[UISPEC_HTTPConnection class]
...
- (BOOL)isAuthenticated
{
	...
	UISPEC_HTTPAuthenticationRequest *auth = [[[UISPEC_HTTPAuthenticationRequest alloc] initWithRequest:request] autorelease];
...
- (BOOL)parseRangeRequest:(NSString *)rangeHeader withContentLength:(UInt64)contentLength
{
	...
	for(i = 0; i < [ranges count] - 1; i++)
	{
		...
		for(j = i+1; j < [ranges count]; j++)
		{
			...
			DDRange iRange = UISPEC_DDIntersectionRange(range1, range2);
...
- (NSObject<HTTPResponse> *)httpResponseForURI:(NSString *)path
{
	...
	{
		return [[[UISPEC_HTTPFileResponse alloc] initWithFilePath:filePath] autorelease];
	}
