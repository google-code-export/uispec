...
NSString *const UISPEC_AsyncSocketException = @"AsyncSocketException";
NSString *const UISPEC_AsyncSocketErrorDomain = @"AsyncSocketErrorDomain";
...
@interface UISPEC_AsyncSocket (Private)
...
**/
@interface UISPEC_AsyncReadPacket : NSObject
{
@public
...
@implementation UISPEC_AsyncReadPacket
...
@interface UISPEC_AsyncWritePacket : NSObject
{
@public
	NSData *buffer;
	CFIndex bytesDone;
...
@implementation UISPEC_AsyncWritePacket
...
@implementation UISPEC_AsyncSocket
...
{
	if (theDelegate == NULL)
		[NSException raise:UISPEC_AsyncSocketException format:@"Attempting to accept without a delegate. Set a delegate first."];
	
	if (theSocket != NULL || theSocket6 != NULL)
		[NSException raise:UISPEC_AsyncSocketException format:@"Attempting to accept while connected or accepting connections. Disconnect first."];
...
- (BOOL)connectToHost:(NSString*)hostname onPort:(UInt16)port error:(NSError **)errPtr
{
	if(theDelegate == NULL)
	{
		...
		[NSException raise:UISPEC_AsyncSocketException format:message];
	}
...
{
	UISPEC_AsyncSocket *newSocket = [[[[self class] alloc] initWithDelegate:theDelegate] autorelease];
...
- (NSError *)getSocketError
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"AsyncSocketCFSocketError",
														 @"UISPEC_AsyncSocket", [NSBundle mainBundle],
														 @"General CFSocket error", nil);
	
	NSDictionary *info = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:UISPEC_AsyncSocketErrorDomain code:AsyncSocketCFSocketError userInfo:info];
}
...
- (NSError *)getAbortError
{
	...
	return [NSError errorWithDomain:UISPEC_AsyncSocketErrorDomain code:AsyncSocketCanceledError userInfo:info];
}
...
- (NSError *)getReadMaxedOutError
{
	...
	return [NSError errorWithDomain:UISPEC_AsyncSocketErrorDomain code:AsyncSocketReadMaxedOutError userInfo:info];
}
...
- (NSError *)getReadTimeoutError
{
	...
	return [NSError errorWithDomain:UISPEC_AsyncSocketErrorDomain code:AsyncSocketReadTimeoutError userInfo:info];
}
...
- (NSError *)getWriteTimeoutError
{
	...
	return [NSError errorWithDomain:UISPEC_AsyncSocketErrorDomain code:AsyncSocketWriteTimeoutError userInfo:info];
}
...
- (void)readDataToLength:(CFIndex)length withTimeout:(NSTimeInterval)timeout tag:(long)tag;
{
	...
	UISPEC_AsyncReadPacket *packet = [[UISPEC_AsyncReadPacket alloc] initWithData:buffer
															timeout:timeout
																tag:tag
												   readAllAvailable:NO
														 terminator:nil
...
- (void)maybeDequeueRead
{
	...
	if(theCurrentRead == nil && [theReadQueue count] != 0 && theReadStream != NULL)
	{
		...
		UISPEC_AsyncReadPacket *newPacket = [theReadQueue objectAtIndex:0];
...
- (void)writeData:(NSData *)data withTimeout:(NSTimeInterval)timeout tag:(long)tag;
{
	...
	UISPEC_AsyncWritePacket *packet = [[UISPEC_AsyncWritePacket alloc] initWithData:data timeout:timeout tag:tag];
...
- (void)maybeDequeueWrite
{
	if (theCurrentWrite == nil && [theWriteQueue count] != 0 && theWriteStream != NULL)
	{
		...
		UISPEC_AsyncWritePacket *newPacket = [theWriteQueue objectAtIndex:0];
...
static void MyCFSocketCallback (CFSocketRef sref, CFSocketCallBackType type, CFDataRef address, const void *pData, void *pInfo)
{
	...
	UISPEC_AsyncSocket *socket = [[(UISPEC_AsyncSocket *)pInfo retain] autorelease];
